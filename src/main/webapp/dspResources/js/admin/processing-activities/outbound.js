...
function populateMessages(fromDate,toDate) {
	...
	aoColumns: [
		...
		"render": function ( data, type, row, meta ) {
			...
			returnData += '<br /><a href="/FileDownload/downloadFile.do?fromPage=outbound&filename='+encodeURIComponent(row.outputFileName)+'&utBatchName='+row.utBatchName+'&foldername=archivesOut&orgId='+row.orgId+'" title="View Original File">Download Outbound File</a>';
		    }
