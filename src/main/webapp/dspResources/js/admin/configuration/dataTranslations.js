...
jQuery(function ($) {
    ...
    $(document).ready(function () {
        ...
        $(document).on('click', '.macroDefinitions', function () {
            $.ajax({
                url: '/administrator/configurations/macroDefinitions',
                type: "GET",
                data: {
                    'macroCategory': 1
                },
        ...
        $('#macro').change(function () {
            ...
            if (selMacro > 0) {
                ...
                if (list.indexOf(selMacro) !== -1) {
                    $.ajax({
                        url: '/administrator/configurations/getMacroDetails.do',
                        type: "GET",
                        data: {'macroId': selMacro},
                        success: function (data) {
                            $('#modalContent').html(data);
                            if(!$('#dataTranslationsModal').hasClass('show')) {
        ...
        $(document).on('click', '.showCrosswalks', function () {
          ...
          if(whichView == 0) {
            $(this).attr('rel',1);
            $(this).html('Show Only Crosswalks Currently Used');
            $('#crosswalkTitle').html("Other Available");
          }
          else {
            $(this).attr('rel',0);
            $(this).html('Show Other Available Crosswalks');
            $('#crosswalkTitle').html("Currently Used");
          }
        });
        ...
        $('#next').click(function () {
            ...
            if(!$(this).hasClass("disabled")) {
                ...
                $.ajax({
                    ...
                    data: {'categoryId': 1},
                    success: function (data) {
                        window.location.href = "scheduling?msg=translationsUpdated";
                    }
                });
            }
            else {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $("#crosswalksTable").find('#cwDataTable').dataTable({
                ...
                "oLanguage": {
                    "sSearch": "_INPUT_",
                    sSearchPlaceholder: 'Filter Crosswalks',
