        ...
        try {
        	...
        	try {
            	try {
                    ...
                    messageDetails.setmessageSubject("processMassBatches job error - " + System.getProperty("server.identity"));
                	...
                	throw new Exception("Error occurred for processMassBatches  - schedule task",ex);
                } catch (Exception ex1) {
                    Logger.getLogger(processMassBatches.class.getName()).log(Level.SEVERE, null, ex1);
                }
            	
            	throw new Exception("Error occurred for processMassBatches job  - new type schedule task",ex);
            } catch (Exception ex1) {
                Logger.getLogger(processMassBatches.class.getName()).log(Level.SEVERE, null, ex1);
            }
        }
